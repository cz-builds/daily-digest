...
<div style="border-bottom:2px solid #111;padding-bottom:12px;margin-bottom:24px;">
  ...
  <p style="margin:4px 0 0;color:#666;font-size:13px;">{{ date }} · {{ items|length }} items across AI, Hardware & Space</p>
</div>

{% for cat, cat_items in grouped.items() %}
<div style="margin-bottom:28px;">
  <h2 style="font-size:14px;text-transform:uppercase;color:#888;letter-spacing:1px;border-bottom:1px solid #eee;padding-bottom:4px;margin:0 0 12px;">{{ cat }}</h2>
  {% for it in cat_items %}
  <div style="margin-bottom:20px;">
    <h3 style="font-size:15px;margin:0 0 4px;">
      <a href="{{ it.url }}" style="color:#111;text-decoration:none;">{{ it.title }}</a>
      <span style="font-size:11px;color:#aaa;font-weight:normal;"> · {{ it.source }}</span>
    </h3>
    <p style="margin:0 0 4px;color:#444;font-size:13px;">{{ it.summary_en or it.summary }}</p>
    {% if it.why_care %}
    <p style="margin:0;color:#2a7ae2;font-size:12px;">→ {{ it.why_care }}</p>
    {% endif %}
  </div>
  {% endfor %}
</div>
...
<p style="font-size:11px;color:#aaa;text-align:center;">
  Auto-curated by LLM from {{ sources_count }} sources. Scored by relevance to AI + Hardware + Space.
</p>
</body>
</html>
"""
...
def render(items):
    ...
    headline = items[0]["title"][:60] if items else "Today"
